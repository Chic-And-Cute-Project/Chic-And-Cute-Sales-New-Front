...
@Component({
  selector: 'app-intake-branch',
  standalone: false,
  templateUrl: './intake-branch.html',
  styleUrl: './intake-branch.css'
})
export class IntakeBranch implements OnInit {
  savingIntakeBranch: boolean = false;
  ...
  refreshInventories(page: number, firstRequest: boolean) {
    if (firstRequest) {
      this.inventoryService.countByBranch(this.supplyBranch.branchId, true).subscribe({
        next: (response) => {
    ...
    this.inventoryService.getAllByBranchAndPage(this.supplyBranch.branchId, page, true).subscribe({
      next: (response) => {
  ...
  searchInventories(page: number, firstRequest: boolean) {
    if (firstRequest) {
      this.inventoryService.countByBranchAndProductCode(this.supplyBranch.branchId, this.productCode, true).subscribe({
        next: (response) => {
          this.inventoriesSize = response.count;
        },
        error: (error: ErrorMessage) => {
    ...
    this.inventoryService.searchInventoriesByBranchAndPage(this.productCode, this.supplyBranch.branchId, page, true).subscribe({
      next: (response) => {
        this.snackBar.dismiss();
        this.inventories = response.inventories;
      },
  ...
  selectProduct(inventoryToAdd: InventoryDto) {
    ...
    if (!elementExisting) {
      let product: SupplyBranchProductDto = { product: inventoryToAdd.product, productId: inventoryToAdd.product.id, quantity: 1, limit: inventoryToAdd.quantity } as SupplyBranchProductDto;
  ...
  saveIntakeGuide() {
    this.savingIntakeBranch = true;
    this.snackBar.open('Alimentando sede');
    this.supplyBranchService.takeInventory(this.supplyBranch).subscribe({
      next: () => {
        this.savingIntakeBranch = false;
        this.snackBar.dismiss();
